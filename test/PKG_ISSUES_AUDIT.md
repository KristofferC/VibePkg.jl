...
- **Fixing progress (worktree-isolated agent per bug, file-partitioned):**
  ...
  - **Deferred (attempted, not merged — need a lower-risk fix):** #3901 (resolver error shows build numbers: fix makes hot `VersionBound` non-`isbits` — needs the `isbits` encoding). Stays `@test_broken`. (#3326 was previously deferred here; fixed in Wave 6.)
  - **Wave 5 — 2 fixed:** #4580 (instantiate now skips artifact downloads and errors on missing package sources under `JULIA_PKG_OFFLINE=1` — `Execution.ensure_artifacts!`/`ensure_sources_installed!` consult `config.offline`) and #4579 (`Registry.update` is a no-op offline) → both testsets promoted to `@test` in `test/pkg_issues.jl`.
  - **Excluded (features / design-calls, left as `@test_broken`):** #1568 (build-metadata version support), #3269 (raw-artifact file modes), #2028 (`semver_spec` consistency). (#708 was previously excluded here; fixed in Wave 6.)
  - **Wave 6 — 2 fixed (2026-07-14 review-fix pass):** #3326 (`find_project_file` now preserves the final project-file symlink and canonicalizes only its parent directory, so the Manifest lands beside the symlink and `Base.project_file_manifest_path` resolves it → `Environments.jl`) and #708 (checkout from the bare clone cache extracts trees with submodule gitlinks explicitly instead of force-checkout, so adding a git package containing a submodule succeeds → `Git.jl`) → both testsets promoted to `@test` in `test/pkg_issues.jl`.
...
| 3326 | Projects with symlinked `Project.toml`s are broken | bug | **FIXED (Wave 6)** | VibePkg's find_project_file (src/Environments.jl:100) resolves a symlinked Project.toml via safe_realpath before anything else — identical to real Pkg (src/Types.jl:236) — so it writes/reads the Manifest next to the symlink TARGET… |
...
| 708 | Pkg.add on git repository with a submodule raises GitError | bug | **FIXED (Wave 6)** | Ran an offline repro through VibePkg's real add-by-url path. Built a local git package repo (valid Project.toml, name=SubModPkg) containing a genuine git submodule (.gitmodules + gitlink entry to a second local repo), then called… |
