...
function showlog(io::IO, rlog::ResolveLog; view::Symbol = :plain)
    ...
    initentries = ResolveLogEntry[event[1]::ResolveLogEntry for event in rlog.init.events]
...
function prune_graph!(graph::Graph)
    ...
    for p0 in findall(.~(pkg_mask))
        # Find the version (exactly one state is allowed for pruned packages)
        s0 = findfirst(gconstr[p0])::Int
