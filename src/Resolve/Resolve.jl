...
using ..Versions
# Minimal-diff port aliases: this module was written against Pkg's Registry
# and Types modules. In VibePkg those roles are played by Registries (the
# multi-registry compat queries) and Stdlibs (the stdlib tables) — the only
# things the resolver actually uses from them.
using ..Registries
using ..Stdlibs
...
function _resolve(graph::Graph, lower_bound::Union{Vector{Int}, Nothing}, previous_sol::Union{Vector{Int}, Nothing})
    ...
    if maxsum_result == :unsat
        ...
        # achieve
        trigger_failure!(graph, sol, staged::Tuple{Int, Int})
    else
